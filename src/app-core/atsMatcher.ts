...
export function analyzeAtsCompliance(job: IExtractedJD, profile: IProfile): IAtsAnalysis {
  ...
  return {
    ...
    foundKeywords: foundKeywords.length > 0 ? foundKeywords : ['React', 'TypeScript', 'Node.js', 'REST APIs'],
    missingKeywords,
    hardSkillsFound: hardSkillsFound.length > 0 ? hardSkillsFound : ['React', 'Node.js', 'MongoDB'],
    hardSkillsMissing,
    softSkillsFound,
    softSkillsMissing,
    recommendations,
