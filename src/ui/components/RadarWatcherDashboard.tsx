...
import {
  ...
  RotateCcw, Check, Phone, Search, Users, ChevronRight, FileText, ArrowRight
} from 'lucide-react';
...
export function RadarWatcherDashboard({ profile, onOpenJob }: RadarWatcherDashboardProps) {
  ...
  const [testChannel, setTestChannel] = useState(config.monitoredChannels[0]?.name || 'Freshershunt - Off Campus Drive Updates');
  const [testPlatform, setTestPlatform] = useState<'whatsapp' | 'telegram'>('telegram');
  const [isProcessing, setIsProcessing] = useState(false);
  
  // Discover / Multi-Select Channels Modal
  ...
  const filteredDiscovered = discoveredList.filter((ch) => {
    ...
    const matchesSearch = ch.name.toLowerCase().includes(discoverSearch.toLowerCase());
    return matchesFilter && matchesSearch;
  });

  const handleToggleSelectChannel = (name: string) => {
  ...
  const handleSelectAllDiscovered = () => {
    const unmonitored = filteredDiscovered
      .filter((ch) => !existingChannelNames.has(ch.name.toLowerCase()))
      .map((ch) => ch.name);
    setSelectedChannels(unmonitored);
  };

  const handleSaveSelectedDiscovered = () => {
    const toAdd = discoveredList.filter((ch) => selectedChannels.includes(ch.name));
    channelManager.bulkAddChannels(toAdd);
    refreshState();
    setSelectedChannels([]);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#121215] border border-[#27272a] rounded-[24px] p-6 shadow-2xl">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="px-3.5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow"
          >
          ...
          <button
            ...
            className="px-3.5 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow"
          >
          ...
          >
            ...
            <span>Select Channels from Socials</span>
          </button>

          <button
            onClick={() => setShowAddModal(true)}
            className="px-3 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-zinc-700 rounded-xl text-xs font-bold transition flex items-center gap-1"
            title="Add Custom URL or Channel Handle"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-[11px] font-mono text-purple-400 hover:text-purple-300 font-bold flex items-center gap-1 transition"
                >
                ...
                <div className="p-8 text-center bg-[#18181b] border border-dashed border-zinc-700 rounded-2xl space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-zinc-400 leading-relaxed font-sans">
                      Select specific placement groups & channels from your connected accounts to monitor.
                    </p>
                  </div>
                  <button
                    onClick={handleOpenDiscoverModal}
          ...
          <div className="p-5 bg-[#121215] border border-[#27272a] rounded-[24px] space-y-3 shadow-xl">
            ...
            <p className="text-[11px] text-zinc-400">
              Paste a WhatsApp/Telegram forward to test live noise triage, deduplication, and pipeline execution.
            </p>
            ...
            <textarea
              ...
              placeholder="Paste raw forward here (e.g. *JioHotstar / Microsoft Recruitment 2026* Role: Full Stack...)"
            ...
            <div className="space-y-3 max-h-[640px] overflow-y-auto pr-1">
              {filteredFeed.length === 0 ? (
                ...
                filteredFeed.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 bg-[#18181b] border border-[#27272a] rounded-2xl space-y-3 transition hover:border-zinc-700"
                  >
                    ...
                    {item.extractedCompany && item.extractedRole && (
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-[#09090b] border border-[#27272a] rounded-xl text-xs">
                        <div>
                          <span className="font-bold text-white">{item.extractedCompany}</span>
                          <span className="text-zinc-500"> — </span>
                          <span className="text-purple-400 font-semibold">{item.extractedRole}</span>
                        </div>

                        {item.jobId && (
                          <button
                            onClick={() => onOpenJob(item.jobId!)}
                            className="px-2.5 py-1 bg-purple-600/30 hover:bg-purple-600 text-purple-300 hover:text-white border border-purple-500/50 rounded-lg text-[11px] font-bold font-mono transition flex items-center gap-1 shrink-0"
                          >
                            <span>View in Job Feed</span>
                    ...
                    <div className="flex items-center justify-between text-[10px] font-mono text-zinc-500 pt-1 border-t border-zinc-800/60">
                      <span>Received at {new Date(item.timestamp).toLocaleTimeString()}</span>
                      <span>Target: MCA 2026 / Full-Stack MERN</span>
                    </div>
                  </div>
                ))
              )}
      ...
      {/* ── 1. DISCOVER & MULTI-SELECT SOCIAL CHANNELS MODAL ── */}
      {showDiscoverModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#121215] border border-[#27272a] rounded-[28px] max-w-2xl w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-white">Select Channels from Your Socials</h3>
                  <p className="text-[11px] text-zinc-400">
                    Check the groups and channels you want JobRadar to monitor in real-time
                  </p>
                </div>
              </div>
              <button onClick={() => setShowDiscoverModal(false)} className="text-zinc-400 hover:text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-[#09090b] border border-[#27272a] rounded-xl text-xs text-white font-mono focus:outline-none focus:border-purple-500"
                />
              </div>

              <div className="flex gap-1 bg-[#09090b] p-1 rounded-xl border border-[#27272a] shrink-0">
                {(['all', 'whatsapp', 'telegram'] as const).map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setDiscoverFilter(tab)}
                    className={`px-3 py-1 rounded-lg text-xs font-bold font-mono transition capitalize ${
                      discoverFilter === tab ? 'bg-zinc-800 text-white' : 'text-zinc-400 hover:text-white'
                    }`}
                  >
                    {tab}
                  </button>
            ...
            <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1">
              {filteredDiscovered.map((ch) => {
                const isAlreadyMonitored = existingChannelNames.has(ch.name.toLowerCase());
                ...
                return (
                  <div
                    key={ch.name}
                    onClick={() => {
                      if (!isAlreadyMonitored) handleToggleSelectChannel(ch.name);
                    }}
                    className={`p-3.5 rounded-2xl border transition flex items-center justify-between gap-3 ${
                      isAlreadyMonitored
                        ? 'bg-[#18181b]/50 border-zinc-800/40 opacity-70 cursor-not-allowed'
                        : isSelected
                        ? 'bg-purple-950/30 border-purple-700/60 cursor-pointer shadow'
                        : 'bg-[#18181b] border-[#27272a] hover:border-zinc-700 cursor-pointer'
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <input
                        type="checkbox"
                        checked={isAlreadyMonitored || isSelected}
                        disabled={isAlreadyMonitored}
                        onChange={() => handleToggleSelectChannel(ch.name)}
                        className="w-4 h-4 rounded text-purple-600 focus:ring-0 focus:outline-none"
                      />
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] font-mono text-zinc-400 flex items-center gap-2">
                          <span>{ch.type === 'group' ? '👥 Group' : '📢 Channel'}</span>
                          {ch.memberCount && <span>• {ch.memberCount.toLocaleString()} members</span>}
                        </p>
                      </div>
                    </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-zinc-800">
              <button
                ...
                className="flex-1 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-bold text-xs rounded-xl transition"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveSelectedDiscovered}
                disabled={selectedChannels.length === 0}
                className="flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:brightness-110 text-white font-bold text-xs rounded-xl transition disabled:opacity-50 flex items-center justify-center gap-1.5 shadow"
              >
                <Check className="w-4 h-4" />
                <span>Monitor Selected ({selectedChannels.length})</span>
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div className="p-5 bg-[#18181b] border border-[#27272a] rounded-2xl space-y-3">
                  ...
                  <p className="text-xs text-zinc-300 leading-relaxed font-sans">
                    Clicking below launches the authentic WhatsApp Web session window. WhatsApp servers render the live dynamic QR code that your phone links to without errors.
                  </p>
                  <button
      ...
      {/* ── 3. TELEGRAM AUTHENTICATION MODAL ── */}
      {showTelegramModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      {/* ── 4. ADD SINGLE CUSTOM CHANNEL MODAL ── */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#121215] border border-[#27272a] rounded-[28px] max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-white">Add Custom Channel</h3>
                  <p className="text-[11px] text-zinc-400">Enter custom channel name or invite link</p>
                </div>
              </div>
