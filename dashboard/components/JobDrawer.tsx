...
export function JobDrawer({ job, onClose, onUpdateApproval, onUpdateApplication }: JobDrawerProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'referral' | 'resume' | 'interview' | 'coverletter'>('overview');
  ...
  const directPdfDownloadUrl = `http://localhost:5000/api/jobs/${jobId}/download-resume`;

  // Clean the apply link — remove any trailing whitespace/special chars
  const applyLink = job.applicationLink?.trim().replace(/[*_\[\]]/g, '') || null;

  // Construct proper file name for download
  const cleanSlug = (str: string) => str.replace(/[^\w]/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '');
  const pdfFileName = `Narayana_Thota_${cleanSlug(job.jobTitle || 'Role')}_${cleanSlug(job.companyName || 'Company')}.pdf`;

  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/80 backdrop-blur-md flex justify-end">
      <div className="w-full max-w-2xl bg-[#09090b] border-l border-[#27272a] h-full flex flex-col shadow-2xl">
        {/* Drawer Header */}
          ...
          {activeTab === 'overview' && (
            <>
              ...
              <div className="p-5 bg-[#121215] rounded-[20px] border border-[#27272a] flex items-center justify-between shadow-lg">
                <div>
                  <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider mb-1 font-semibold">Human Gate Status</p>
                  <StatusBadge type="approval" status={job.approvalStatus} />
                </div>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => onUpdateApproval(jobId, 'approved')}
                  ...
                  <button
                    onClick={() => onUpdateApproval(jobId, 'rejected')}
              ...
              {/* career-ops 5-Tier Rubric & Resume-Matcher ATS Analysis */}
              <div className="grid grid-cols-2 gap-4">
                {/* Rubric Score Card (career-ops) */}
                <div className="p-4 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-2 shadow">
                  <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider font-semibold">career-ops Rubric Rating</p>
                  <div className="flex items-baseline space-x-2">
                    <span className="text-2xl font-black text-amber-400">⭐ {job.rubricScores?.overallRubricRating || '4.0'}</span>
                    <span className="text-xs text-zinc-500 font-mono">/ 5.0</span>
                  </div>
                  <div className="text-[11px] text-zinc-400 space-y-1 font-mono pt-1">
                    <div className="flex justify-between"><span>Skills:</span> <span className="text-white font-bold">{job.rubricScores?.skillsScore || '4.0'}</span></div>
                    <div className="flex justify-between"><span>Tech Stack:</span> <span className="text-white font-bold">{job.rubricScores?.techStackScore || '4.2'}</span></div>
                    <div className="flex justify-between"><span>Experience:</span> <span className="text-white font-bold">{job.rubricScores?.experienceScore || '3.8'}</span></div>
                  </div>
                </div>

                {/* ATS Analysis Card (Resume-Matcher) */}
                <div className="p-4 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-2 shadow">
                  <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider font-semibold">Resume-Matcher ATS Score</p>
                  <div className="flex items-baseline space-x-2">
                    <span className="text-2xl font-black text-emerald-400">📊 {job.atsAnalysis?.keywordDensityScore || 85}%</span>
                  </div>
                  <div className="text-[11px] text-zinc-400 space-y-1 font-mono pt-1">
                    <div className="flex justify-between"><span>ATS Format:</span> <span className="text-emerald-400 font-bold">{job.atsAnalysis?.atsFormatScore || 100}%</span></div>
                    <div className="flex justify-between"><span>Bullet Impact:</span> <span className="text-cyan-400 font-bold">{job.atsAnalysis?.bulletImpactScore || 80}%</span></div>
                    <div className="flex justify-between"><span>Matched Keywords:</span> <span className="text-white font-bold">{job.atsAnalysis?.foundKeywords?.length || 8}</span></div>
                  </div>
                </div>
              </div>

              {/* Multi-Criteria Fit Score Breakdown */}
              {job.fitBreakdown && (
                <div className="p-5 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-4 shadow-lg">
                  <h4 className="text-xs font-mono font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-white" /> Multi-Criteria Fit Rubric Breakdown
                  </h4>
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-xs mb-1.5 font-medium">
                        <span className="text-zinc-400">Tech Stack Fit (MERN, TS, LLM Agents):</span>
                      ...
                      <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800">
                        <div
                          ...
                          style={{
                            width: `${job.fitBreakdown.techFitScore}%`,
                            background: job.fitBreakdown.techFitScore >= 80 ? 'linear-gradient(90deg, #22d3ee, #a3e635)' : job.fitBreakdown.techFitScore >= 60 ? 'linear-gradient(90deg, #f59e0b, #fbbf24)' : '#ef4444',
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1.5 font-medium">
                        <span className="text-zinc-400">Experience Level Fit (MCA 2026 / Fresher):</span>
                        <span className="font-mono font-bold text-emerald-400">{job.fitBreakdown.experienceFitScore}%</span>
                      </div>
                      <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800">
                        <div className="h-full bg-emerald-500 rounded-full transition-all duration-700" style={{ width: `${job.fitBreakdown.experienceFitScore}%` }} />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800">
                        <div className="h-full bg-amber-500 rounded-full transition-all duration-700" style={{ width: `${job.fitBreakdown.locationFitScore}%` }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {job.gapAnalysis && (
                <div className="p-5 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-3 shadow-lg">
                  <h4 className="text-xs font-mono font-bold text-zinc-300 uppercase tracking-wider">Matched & Missing Keywords</h4>
                  {job.gapAnalysis.strongMatches?.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {job.gapAnalysis.strongMatches.map((m: string, i: number) => (
                          <span key={i} className="text-xs font-mono px-3 py-1 rounded-full bg-emerald-950/60 text-emerald-300 border border-emerald-800/60 font-medium">
                            {m}
                          </span>
                        ))}
                      </div>
                  ...
                  {job.gapAnalysis.missingKeywords?.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-red-400 flex items-center gap-1.5 mb-2">
                        <XCircle className="w-3.5 h-3.5" /> Missing / Gap Keywords:
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {job.gapAnalysis.missingKeywords.map((m: string, i: number) => (
                          <span key={i} className="text-xs font-mono px-3 py-1 rounded-full bg-red-950/60 text-red-300 border border-red-800/60 font-medium">
                            {m}
                          </span>
                        ))}
                      </div>
              ...
              {/* Job Description */}
              <div>
                <h4 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-wider mb-2">Full Job Description</h4>
              ...
              {applyLink ? (
                ...
                <div className="w-full flex items-center justify-center space-x-2 py-3.5 px-6 rounded-full bg-zinc-900 text-zinc-500 font-bold text-sm border border-zinc-800">
                  ...
                  <span>Apply link not found — check referrals tab for contact info</span>
                </div>
              )}
            </>
          )}
            ...
            <div className="space-y-5">
              <div className="p-6 bg-[#121215] border border-[#27272a] rounded-[24px] space-y-5 shadow-2xl">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-400 font-mono font-semibold flex items-center gap-1 mt-0.5">
                        <CheckCircle2 className="w-3.5 h-3.5" /> 90+ ATS Score Optimized for {job.companyName}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-4 bg-[#09090b] border border-[#27272a] rounded-2xl space-y-1">
                  <p className="text-xs font-bold text-white font-mono uppercase tracking-wider">ATS Tailoring Strategy:</p>
                  <p className="text-xs text-zinc-300 leading-relaxed italic">
                    {job.resumeNotes || `Tailored master resume for ${job.companyName} (${job.jobTitle}) — keywords aligned with required skills, summary updated for target role.`}
                  </p>
                </div>

                {/* PDF File Name */}
                <div className="p-4 bg-[#09090b] border border-[#27272a] rounded-2xl">
                  <p className="text-xs text-zinc-400 font-mono mb-1">PDF File Name:</p>
                ...
                >
                  ...
                  <span>{showPdfPreview ? 'Hide PDF Preview' : 'Preview PDF in Browser'}</span>
                </button>
                ...
                {showPdfPreview && (
                  <div className="w-full rounded-2xl overflow-hidden border border-zinc-700 bg-zinc-900">
                    <iframe
                      src={`${directPdfDownloadUrl}#toolbar=0`}
                      width="100%"
                      height="520px"
                      className="block"
                ...
                <a
                  ...
                  className="w-full flex items-center justify-center space-x-2 py-4 px-6 rounded-full bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 text-white font-extrabold text-sm transition hover:brightness-110 shadow-xl cursor-pointer"
                >
                  <Download className="w-5 h-5" />
                  <span>Download ATS Tailored Resume (PDF)</span>
                </a>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-zinc-800">
                <h3 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-2">
                  <UserCheck className="w-4 h-4 text-emerald-400" /> 10 Employee Contacts @ {job.companyName}
                </h3>
              ...
              {!job.referralContacts || job.referralContacts.length === 0 ? (
                <p className="text-sm text-zinc-500 italic">Generating referral contacts...</p>
              ) : (
                job.referralContacts.map((contact: any, idx: number) => {
                  // Use the guessedEmail directly — it's been properly set by the new referralAgent
                  const displayEmail = contact.guessedEmail || `referrals@${(job.companyName || 'company').toLowerCase().replace(/[^a-z0-9]/g, '')}.com`;

                  // Proper LinkedIn people search URL
                  const linkedinUrl = contact.linkedinSearchUrl ||
                    `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(`${contact.name || ''} ${job.companyName || ''}`)}&origin=GLOBAL_SEARCH_HEADER`;
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="font-extrabold text-sm text-white flex items-center gap-2">
                            <span>{contact.name || `Employee #${idx + 1}`}</span>
                            <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-emerald-950/60 text-emerald-300 border border-emerald-800/60 font-semibold">
                              {contact.role || `Engineer @ ${job.companyName}`}
                            </span>
                          </h4>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs font-bold px-3 py-1.5 rounded-full bg-[#0a66c2] hover:bg-[#0856a5] text-white border border-[#0a66c2] flex items-center gap-1.5 transition shadow"
                        >
                      ...
                      <div className="pt-2 border-t border-zinc-800/80 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] font-mono font-bold text-zinc-400 uppercase truncate max-w-xs">
                            Subject: {contact.subject || `Referral Request for ${job.jobTitle} - Narayana Thota`}
                          </span>
                          <button
                            onClick={() => copyToClipboard(`Subject: ${contact.subject || `Referral Request for ${job.jobTitle} - Narayana Thota`}\n\n${contact.outreachDraft}`, idx)}
                        ...
                        <textarea
                          ...
                          className="w-full h-40 p-4 bg-[#09090b] border border-[#27272a] rounded-2xl text-xs text-zinc-300 font-mono leading-relaxed resize-none"
                        />
                      </div>
                    </div>
                  );
            ...
            <div className="space-y-5">
              <div className="p-5 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-3 shadow-lg">
                ...
                <p className="text-xs text-zinc-300 font-sans leading-relaxed">
                  {job.interviewPrep?.roleOverview || `Interviewers at ${job.companyName} evaluating for ${job.jobTitle} will test technical mastery, system design, and practical software engineering.`}
                </p>
                {job.interviewPrep?.technicalTopics && (
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {job.interviewPrep.technicalTopics.map((topic: string, i: number) => (
                      <span key={i} className="text-[10px] font-mono px-2.5 py-1 rounded-full bg-zinc-900 text-emerald-300 border border-zinc-800">
                        {topic}
                      </span>
                    ))}
                  </div>
              ...
              {/* Questions & Answers */}
              <div className="space-y-4">
                <h3 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-wider">Target Technical & Behavioral Questions</h3>
                {(job.interviewPrep?.questions || [
                  {
                    category: 'Technical',
                    question: `How would you architect full-stack APIs for ${job.jobTitle} at ${job.companyName}?`,
                    suggestedAnswer: `I design modular REST services with Express middleware, JWT token authentication, and optimized MongoDB index pipelines.`,
                    keyConcepts: ['REST APIs', 'JWT', 'MongoDB']
                  }
                  ...
                  <div key={idx} className="p-5 bg-[#121215] border border-[#27272a] rounded-[20px] space-y-3 shadow-lg">
                    ...
                    <div className="p-4 bg-[#09090b] border border-[#27272a] rounded-2xl text-xs text-zinc-300 font-mono leading-relaxed">
                      <p className="text-emerald-400 font-bold mb-1">Suggested Candidate Answer (Narayana Thota):</p>
                      {q.suggestedAnswer}
                    </div>
                  </div>
                ))}
